...
## 4. 拉代码
仓库为 **private**，服务器走 SSH + 只读 Deploy Key（配置见 §10「VPS 拉取认证」）：
```bash
cd /home/france-infor/htdocs/infr.europeanaialliance.org
git clone git@github.com:Ayacloud-KEWEN/franceinfor.git .
```
...
### VPS 拉取认证：只读 Deploy Key（2026-07-10 重建，当前方案）
仓库为 **private**，服务器用一把**只读** Deploy Key 拉代码：

```bash
# 1. 生成专用密钥（独立文件名，别复用 root 默认密钥）
ssh-keygen -t ed25519 -f ~/.ssh/github_deploy -N "" -C "deploy@francego"
cat ~/.ssh/github_deploy.pub

# 2. GitHub 仓库 → Settings → Deploy keys → Add deploy key
#    粘贴公钥；⚠️ "Allow write access" 不要勾（保持只读）
...
# 4. 验证 + 确认 remote 是 SSH 地址
ssh -T git@github.com    # 应显示 successfully authenticated
git remote set-url origin git@github.com:Ayacloud-KEWEN/franceinfor.git
```

> **认知纠偏**：2026-07 的挖矿入侵入口是 **SSH root 密码爆破**（见 §13），与 Deploy Key 无关。Deploy Key 放在 GitHub 的只是公钥，方向单向（服务器→读仓库），不可能被用来攻击服务器；只读授权还保证即使服务器再被攻破，代码仓库也无法被篡改。
>
> **踩坑记录（2026-07-10）**：`git pull` 报 `git@github.com: Permission denied (publickey)` 时，把仓库改成 public 是**没用的**——remote 是 SSH 地址就必须走密钥认证；public 只影响 HTTPS 匿名读取。当时曾临时改 public + HTTPS 拉取，随后已改回 **private + 上述只读 Deploy Key**（商业代码与运维文档不应公开）。

### 被入侵后已撤销的旧部署密钥（历史）
原自动部署用过两把 key：`~/.ssh/gh_actions`（GitHub→VPS）、`~/.ssh/github_deploy`（VPS→GitHub deploy key）。2026-07 服务器遭 root 级入侵后，**这两把及 `~/.ssh/id_ed25519` 均视为泄露、已在 GitHub 撤销**；现行的 `~/.ssh/github_deploy` 为 2026-07-10 重新生成的只读 key（见上）。
