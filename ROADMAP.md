...
- ◐ **Stripe 真实订阅**(Professional €99 / Business €299）。已接入代码框架；上线需在服务器配 `STRIPE_*` 密钥并在 Stripe 后台建 Product/Price + Webhook。见 `CONTINUE.md`。
- ☐ **增值服务留资闭环**：公司/商标注册的 CTA 改为留资表单(姓名/公司/需求)→ 后台线索列表 + 邮件通知 → 你/合作伙伴接单。**最快变现点**。
- ☐ **关注列表 / 轻量 CRM**：收藏企业/招标/采购方，打标签、备注、阶段(潜在→接触→商谈)。`SavedItem` 表已存在，补 UI。
