...
- ☑ **关注列表 / 轻量 CRM**：`/watchlist` 按阶段看板(潜在→接触→商谈→赢单/丢单)，每项可改阶段、加备注、移除。收藏按钮已接入企业搜索结果 + 企业详情页。`SavedItem` 已扩展 `note/stage/tags`。**待扩展**：招标/机会/联系人也加收藏按钮(`SaveButton` 已通用，传 type 即可)、标签编辑 UI、拖拽换列。
