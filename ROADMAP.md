...
- ☑ **分行业落地合规清单** `/compliance`：按行业(食品/健康/建筑/金融/化妆品/软件/零售/能源/通用)给出 法律形式 · TVA/税务 · 雇佣法 · 行业监管认证(CE/HACCP/ACPR/Qualibat…) · GDPR；内容为**人工整理的真实法规事实(非 LLM 生成)** + 免责声明;底部接注册留资 CTA → 「咨询+落地」组合。代码 `lib/data/compliance.ts`、`components/compliance/`。**待办**：内容多语化(目前清单正文为英文)、按公司画像自动选行业、PDF 导出清单。
